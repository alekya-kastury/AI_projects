...
FOR MY_ROW in  (SELECT DISTINCT R.ROOM_ID,R.ROOM_NUMBER, T.ROOM_TYPE, T.ROOM_COST, t.max_persons FROM ROOM R INNER JOIN room_type T ON r.room_type_id=t.room_type_id WHERE R.HOTEL_ID=HOTEL_ID1 and r.room_availability='Y') 
LOOP
...
dbms_output.put_line('**********************************************************************************************');
FOR MY_ROW in  (SELECT ROOM_SERVICE_ID, ROOM_SERVICE_DESCRIPTION, ROOM_SERVICE_COST FROM ROOM_SERVICE R WHERE R.HOTEL_ID=HOTEL_ID1) 
LOOP
...
dbms_output.put_line('**********************************************************************************************');
FOR MY_ROW IN (SELECT PACKAGE_ID, PACKAGE_DESCRIPTION, PACKAGE_COST FROM PACKAGE_DEALS) LOOP
dbms_output.put_line('');
